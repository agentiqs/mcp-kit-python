...
from mcp_kit.targets.registry import RegistryTarget
...
async def main():
    registry_target = RegistryTarget(name="RegistryTarget", registry_url="http://localhost:8000/resolve", context={})

    async with ProxyMCP(target=registry_target).openai_agents_mcp_server() as odoo_mock_mcp:
